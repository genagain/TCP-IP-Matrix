In this assignment, I wrote a Matlab script that builds a TCP/IP communication between two Matlab sessions.  In one session the server script is executed and in the other session the client script is executed.  Every three seconds, a random NxM matrix is generated in the server session and being transmitted to the client session.  The client session receives the matrices from the server and then visualizes the data.  The data visualization is updated periodically as new data is being received.  
